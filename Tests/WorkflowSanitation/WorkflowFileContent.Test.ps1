...
Describe "PreGateCheck in PullRequestHandler should use runs-on: windows-2025" {
    It 'Check PullRequestHandler.yaml for runs-on: windows-2025' {
        # Check that PullRequestHandler.yaml in both templates uses runs-on: windows-2025, which doesn't get updated by the Update AL-Go System Files action
        ...
        foreach($template in @('Per Tenant Extension','AppSource App')) {
            ...
            $yaml.Get('jobs:/PregateCheck:/runs-on').content | Should -Be 'runs-on: windows-2025' -Because "PreGateCheck in $template/PullRequestHandler.yaml should use runs-on: windows-2025"
        }
    }
}
